...
export default {
  ...
  data() {
    return {
      ...
      trackingDict: {
        "20180613_AF_MK_DHB_RP_142_6_processed": "fuchsia", 
        "20180613_AF_MK_DHB_RP_139_4_processed": "steelblue", 
        "20180613_AF_MK_DHB_RP_139_2_processed": "navy", 
        "20180613_AF_MK_DHB_RP_139_1_processed": "coral"
      },
  ...
  methods: {
    ...
    drawPoints(pointsArray, pointsFillColors, ctx) {
      ...
      if (pointsFillColors.length === 1) {
        ...
        pointsArray.forEach(point => {
          ctx.fillStyle = this.trackingDict[point.dataset];
          let cx = this.xScale(point.x) - this.zoomedPointSize / 2;
          let cy = this.yScale(point.y) - this.zoomedPointSize / 2;
          ctx.fillRect(cx, cy, this.zoomedPointSize, this.zoomedPointSize);
        });
        console.log(this.trackingDict)
      }
      ctx.restore();
    },
